...
class ShadowBeanGenerator{
    ...
    fun generateModel(holder: EBeanModel): FileSpec{
        val typeBuilder = TypeSpec.classBuilder(holder.generatedClazzSimpleName).addModifiers(KModifier.PUBLIC).superclass(ClassName(holder.`package`, holder.sourceClazzSimpleName)).addSuperinterface(TypeUtil.activityRefered()).addSuperinterface(TypeUtil.beanScope())
